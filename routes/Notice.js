...
router.get('/', async function(req, res, next) {   
    try { //alert("sdsd");
        const result = await Notice.find({}).populate('notice_type_id').populate('college_id').populate('course_type_id');
...
router.get('/:id', async function(req, res, next) {
    ...
    try {
        ...
        const result = await Notice.findOne({"_id": id}).populate('notice_type_id').populate('college_id').populate('course_type_id');
...
router.put('/:id', async function(req, res, next) {
    ...
    try {
        ...
        const result = await Notice.findOneAndUpdate({"_id": id},doc, options);
...
router.delete('/:id/:status', async function(req, res, next) {
    ...
    try {
        const options = { returnDocument: 'after' };
        const result = await Notice.findOneAndUpdate({"_id": id},doc);
